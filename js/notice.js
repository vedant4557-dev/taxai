...
function buildWhatsNext(){
  ...
  if(bal>0){
    steps.push({
      ...
      title:`File ITR to claim ₹${window.fmt(bal)} refund`,
    ...
    steps.push({
      ...
      title:`Pay ₹${window.fmt(Math.abs(bal))} Self-Assessment Tax now`,
  ...
  if(win==='old'&&window._o.deds){
    ...
    if(unused>5000){
      steps.push({
        ...
        title:`Invest ₹${window.fmt(unused)} more to max your 80C`,
        desc:`You have ₹${window.fmt(unused)} of unused 80C room. PPF, ELSS mutual funds, or NPS contributions qualify.`,
        cta:null
      });
    }
  }
  ...
  steps.push({
    ...
    cta:{label:'⬇ Download Report',action:'window.downloadReport()'}
  });
  ...
  const edgeFlags = window.detectEdgeCases();
...
function buildHeroResult(){
  ...
  if(bal>0){
    tips.push({ico:'💚',text:`You have a <strong>refund of ${window.fmt(bal)}</strong>. File ITR before July 31 to claim it — deposited directly to your bank.`});
  } else if(bal<0){
    tips.push({ico:'🔴',text:`<strong>${window.fmt(Math.abs(bal))} additional tax due</strong>. Pay via Challan 280 on incometax.gov.in before filing to avoid interest under Sec 234B.`});
  } else {
  ...
  if(sav>2000){
    ...
    tips.push({ico:'💡',text:`Switch to <strong>${win==='new'?'New':'Old'} Regime saves ₹${window.fmt(sav)}</strong>. ${loser} Regime costs more for your income profile.`});
  } else {
    tips.push({ico:'⚖️',text:`Both regimes are almost identical for you (diff: ${window.fmt(sav)}). Stick with ${win==='new'?'New':'Old'} Regime.`});
  }
  ...
  if(d){
    ...
    if(unused80c>10000) tips.push({ico:'📈',text:`You have <strong>₹${window.fmt(unused80c)} unused 80C room</strong>. Invest in PPF, ELSS or NPS to reduce taxable income further.`});
    else if(unusedNps>10000) tips.push({ico:'📈',text:`<strong>₹${window.fmt(unusedNps)} unused NPS 80CCD(1B) room</strong>. Extra ₹50K deduction separate from 80C.`});
  ...
  if(!hasErrors && bigRefund && !bal<0){
    emotionalHeadline = `🎉 You're getting ₹${window.fmt(bal)} back!`;
    ...
    emotionalHeadline = `Tax due: ₹${window.fmt(Math.abs(bal))}`;
    ...
    emotionalSub = bigSaving ? `Saves you ₹${window.fmt(sav)} vs the other regime.` : `Both regimes are similar for your income.`;
    emotionalColor = '#2d6a4f';
  }
  ...
  panel.innerHTML=`
    ...
    <div class="hero-winner mb14">
      ...
      <div class="hero-saves">${sav>1000?`Saves ₹${window.fmt(sav)} vs ${win==='new'?'Old':'New'} Regime`:'Both regimes are close'}</div>
    </div>
    <div class="hero-numbers">
      <div class="hero-num">
        <div class="hn-val">${window.fmt(bestTax)}</div>
      ...
      <div class="hero-num ${bal>=0?'refund':'due'}">
        <div class="hn-val">${bal>=0?'+':'-'}${window.fmt(Math.abs(bal))}</div>
...
function updateEmpSummary(){
  ...
  if(s1)s1.textContent=window.fmt(e1);
  if(s2)s2.textContent=window.fmt(e2);
  if(st){
    st.textContent=window.fmt(total);
    st.style.color=total>0?'var(--accent)':'var(--muted)';
  }
}
// Hook into the window.fa() oninput calls for emp salary fields
...
if(typeof buildAuditTrail!=="undefined") window.buildAuditTrail=buildAuditTrail;
if(typeof buildWhatsNext!=="undefined") window.buildWhatsNext=buildWhatsNext;
if(typeof buildHeroResult!=="undefined") window.buildHeroResult=buildHeroResult;
