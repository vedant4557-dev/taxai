...
function updateSim() {
  if (!window._o || !window._n || !window._i) return;

  const slider = document.getElementById('sim-slider');
  const amount = parseInt(slider.value) || 0;
  const cfg = SIM_CONFIG[_simKey];

  document.getElementById('sim-input-val').textContent = '₹' + window.toIN(amount);

  // Determine current winning regime
  const currentWin = window._o.tax <= window._n.tax ? 'old' : 'new';

  // Calculate extra deduction this investment gives
  const extraDeduction = cfg.calcDeduction(amount, window._i, currentWin);
  ...
  if (extraDeduction > 0) {
    ...
    const newInputs = {..._i};
    if (_simKey === 'nps') newInputs.nps = (window._i.nps||0) + amount;
    else if (_simKey === '80c') newInputs.sec80c = (window._i.sec80c||0) + amount;
    else if (_simKey === '80d') newInputs.sec80d_self = (window._i.sec80d_self||0) + amount;
    else if (_simKey === 'hloan') newInputs.home_loan_interest = (window._i.home_loan_interest||0) + amount;

    const newOld = window.compOld(newInputs);
    const newNew = window.compNew(newInputs);
    const oldBest = Math.min(window._o.tax, window._n.tax);
  ...
  // Update UI
  document.getElementById('sim-tax-saved').textContent = taxSaved > 0 ? '₹' + window.toIN(taxSaved) : '₹0';
  document.getElementById('sim-eff-return').textContent = effReturn.toFixed(1) + '%';
  document.getElementById('sim-net-cost').textContent = '₹' + window.toIN(netCost);
  ...
  if (amount === 0) {
    ...
    narrative = `${name}, investing <strong>₹${window.toIN(amount)}</strong> in ${cfg.description} saves you <strong>₹${window.toIN(taxSaved)} in tax</strong> — an instant ${effReturn.toFixed(1)}% return before any investment gains. Your actual out-of-pocket cost is just <strong>₹${window.toIN(netCost)}</strong>. ${cfg.tip}`;
  } else {
  ...
  if (taxSaved > 0) {
    cta.textContent = `✓ Add ₹${window.toIN(amount)} to my ${cfg.label} plan`;
...
function simApply() {
  ...
  setTimeout(() => {
    cta.textContent = `✓ Add ₹${window.toIN(amount)} to my ${SIM_CONFIG[_simKey].label} plan`;
...
function initSim() {
  ...
  const regime = window._o.tax <= window._n.tax ? 'old' : 'new';
  const grossSalary = window._i.gross_salary || 0;
  ...
  const opportunities = Object.entries(SIM_CONFIG).map(([key, cfg]) => {
    ...
    if (key === '80c') {
      const used = Math.min((window._i.sec80c||0)+(window._i.epf_employee||0), 150000);
      optimalAmt = Math.max(0, 150000 - used);
    } else if (key === '80d') {
      const used = (window._i.sec80d_self||0)+(window._i.sec80d_parents||0);
      optimalAmt = Math.max(0, Math.min(25000, 75000 - used));
    } else if (key === 'nps') {
      optimalAmt = Math.max(0, 50000 - (window._i.nps||0));
    } else if (key === 'hloan') {
      optimalAmt = Math.max(0, 200000 - (window._i.home_loan_interest||0));
    }

    if (optimalAmt === 0) return { key, saved: 0, optimalAmt: 0, effReturn: 0 };

    const deduction = cfg.calcDeduction(optimalAmt, window._i, regime);
    if (deduction === 0) return { key, saved: 0, optimalAmt, effReturn: 0 };

    const newInputs = {..._i};
    if (key === 'nps') newInputs.nps = (window._i.nps||0) + optimalAmt;
    else if (key === '80c') newInputs.sec80c = (window._i.sec80c||0) + optimalAmt;
    else if (key === '80d') newInputs.sec80d_self = (window._i.sec80d_self||0) + optimalAmt;
    else if (key === 'hloan') newInputs.home_loan_interest = (window._i.home_loan_interest||0) + optimalAmt;
  ...
  if (!best || best.saved === 0) {
    ...
    let aiMsg = '';
    if (best.key === 'nps') {
      aiMsg = `Invest ₹${window.toIN(best.optimalAmt)} in NPS → save ₹${window.toIN(best.saved)} tax instantly. That's a ${best.effReturn.toFixed(1)}% guaranteed return before any market gains.`;
    } else if (best.key === '80c') {
      const used = Math.min((window._i.sec80c||0)+(window._i.epf_employee||0), 150000);
      aiMsg = `You've used ₹${window.toIN(used)} of your ₹1,50,000 80C limit. Invest ₹${window.toIN(best.optimalAmt)} more in PPF/ELSS → save ₹${window.toIN(best.saved)} tax (${best.effReturn.toFixed(1)}% instant return).`;
    } else if (best.key === '80d') {
      aiMsg = `Buy ₹${window.toIN(best.optimalAmt)}/yr health insurance → save ₹${window.toIN(best.saved)} tax. Net cost after tax savings: just ₹${window.toIN(best.optimalAmt - best.saved)}.`;
    } else if (best.key === 'hloan') {
      aiMsg = `₹${window.toIN(best.optimalAmt)} unused home loan deduction room. Using it saves ₹${window.toIN(best.saved)} in tax.`;
    }
